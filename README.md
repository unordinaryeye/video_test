...
현재 `model_server/Dockerfile`은 **PyTorch 2.5.1 + CUDA 12.4 휠**을 명시적으로 설치하여
호스트 NVIDIA 드라이버 **550 이상**에서 GPU 추론이 자동 활성화되도록 구성되어 있습니다.

### 검증 결과 (GTX 1660 Ti, Driver 551.86)

| 항목 | CPU 폴백 | GPU (cu124) |
|------|---------|------------|
| YOLOv8n 추론 지연 (640x480) | 200~1000 ms | **35~100 ms** (평균 70 ms) |
| End-to-end HTTP 왕복 | - | **~96 ms** |

### 작동 원리

`Dockerfile`에서 PyTorch를 ultralytics보다 **먼저** cu124 휠로 고정 설치합니다.
이를 통해 ultralytics 의존성 해소 단계에서 최신 cu130 휠을 끌어와 호스트 드라이버와
불일치가 발생하는 문제를 차단합니다.
...
모델 서버가 GPU를 잡았는지는 `/health` 응답에서 확인할 수 있습니다.

```bash
curl http://localhost:8001/health
# {"status":"ok","model_loaded":true,"device":"cuda",
#  "device_name":"NVIDIA GeForce GTX 1660 Ti",
#  "torch_version":"2.5.1+cu124","cuda_version":"12.4"}
```

`device`가 `cpu`로 나오면 CPU 폴백 상태입니다. 다음을 확인하세요:
- 호스트 `nvidia-smi`에서 CUDA Version 12.4 이상 표시 여부
- `docker-compose.yml`의 `deploy.resources.reservations.devices` (nvidia, gpu) 설정 유무
- WSL2의 경우 Docker Desktop의 GPU 지원 활성화 상태
...
## 실시간성 정책 (latest-only 모드)

실시간 모니터링 용도이므로 **지연 누적보다 최신 프레임 유지를 우선**합니다.

### 동작 방식

- `config.queue_max_size = 2` — capture↔inference, inference↔sink 큐 크기를 매우 작게 유지
- 큐가 가득 차면 capture는 **가장 오래된 프레임을 버리고** 최신을 넣음
- inference/sink 워커는 한 항목을 꺼낸 뒤 큐에 더 새 항목이 있으면 **이전 것을 버리고 최신만 처리**

이 정책으로 추론 속도가 캡처 속도보다 느려도 화면에 표시되는 프레임이 항상 "방금"에 가깝게 유지되며, 큐가 채워져 지연이 누적되지 않습니다.

### 드롭 모니터링

웹 뷰어의 통계 카드에서 단계별 드롭 수를 실시간으로 확인할 수 있습니다.

| 단계 | 의미 |
|------|------|
| `capture` | 입력 큐가 가득 차서 캡처가 버린 프레임 |
| `inference` | 추론 워커가 latest-only 정책으로 건너뛴 중간 프레임 |
| `sink` | sink 워커가 latest-only 정책으로 건너뛴 결과 |

또는 `/stats` API로 직접 조회:
```bash
curl http://localhost:8080/stats
# {"frame_count":181, "fps":13.7,
#  "drops":{"capture":19, "inference":122, "sink":0}, ...}
```

### 해석 가이드

- **inference 드롭률이 매우 높다** (>80%): GPU 추론이 캡처 fps를 못 따라감 — fps를 낮추거나 더 작은 모델 사용
- **capture 드롭이 inference보다 큼**: capture가 inference보다 너무 빠름 — 정상 (보호 동작)
- **sink 드롭이 발생**: sink가 디스크 I/O 등으로 느려졌다는 신호 — sink 측 점검 필요
